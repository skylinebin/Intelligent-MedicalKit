...
//初始化PB5和PE5,PE6,PE7,PE8为输出口.并使能这两个口的时钟		    
//LED IO初始化
void LED_Init(void)
{
 ...
 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;				 //LED0-->PA.2端口配置
 ...
 GPIO_Init(GPIOE, &GPIO_InitStructure);	  				 //推挽输出 ，IO口速度为50MHz
 GPIO_SetBits(GPIOE,GPIO_Pin_7); 						 //PE.5 输出高 
 
 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;	    		 //LED1-->PE.5 端口配置, 推挽输出
 GPIO_Init(GPIOE, &GPIO_InitStructure);	  				 //推挽输出 ，IO口速度为50MHz
 GPIO_SetBits(GPIOE,GPIO_Pin_8); 						 //PE.5 输出高 
}
